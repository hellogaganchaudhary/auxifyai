# =============================================================================
# Production environment root — composes the reusable modules under
# ../../modules into the full Auxify stack for the production environment
# (Req 42.1). Sizing/HA knobs are set via variables + terraform.tfvars.
#
# Architecture mirrors the local docker-compose stack (Postgres+pgvector, Redis,
# S3 object store) so local and cloud share one architecture (Req 42.2).
# =============================================================================
...
module "network" {
  ...
  single_nat_gateway = false # one NAT per AZ for full HA egress in production
...
# -----------------------------------------------------------------------------
# Database — RDS PostgreSQL + pgvector, Multi-AZ in production (Req 39.5)
# -----------------------------------------------------------------------------
module "database" {
...
# -----------------------------------------------------------------------------
# Cache — ElastiCache Redis, Multi-AZ in production (Req 39.5)
# -----------------------------------------------------------------------------
module "cache" {
