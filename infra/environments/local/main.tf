# =============================================================================
# Local (cloud sandbox) environment root — composes the SAME reusable modules
# under ../../modules as staging/production (Req 42.1). Sizing is dialled down to
# mirror the docker-compose footprint (one Postgres+pgvector, one Redis, one
# object store, one API task) so the architecture that runs locally is the same
# architecture that runs in the cloud — no architectural changes (Req 42.2).
#
# NOTE: day-to-day local development uses ../../../docker-compose.yml. This root
# lets the cloud topology be exercised cheaply in a dev account when needed.
# =============================================================================
...
module "network" {
  ...
  single_nat_gateway = true # single NAT for the cheap sandbox
...
# -----------------------------------------------------------------------------
# Database — RDS PostgreSQL + pgvector (single-AZ sandbox, mirrors compose)
# -----------------------------------------------------------------------------
module "database" {
  ...
  deletion_protection   = false # sandbox is disposable
...
# -----------------------------------------------------------------------------
# Cache — ElastiCache Redis (single-node sandbox, mirrors compose)
# -----------------------------------------------------------------------------
module "cache" {
...
# -----------------------------------------------------------------------------
# Object store — S3 mirrors the local MinIO bucket (Req 42.2)
# -----------------------------------------------------------------------------
module "object_store" {
...
# Application secret bundle — populated out-of-band. Created empty so the app can
# read a stable ARN. No secrets in code.
...
module "api" {
  ...
  # Autoscaling still wired even in the sandbox (Req 39.6)
